...
class driftDiffusionSimulatorBase:
    # no tip class; Emin/Npart parameters
	'''
	...
	def __init__(self):
		...
		self.updateBody()
		self.updateNparticles(self.Npart)
		self.findOverlaps()
	...
	def updateNparticles(self,Npart_in):
	...
	def timestep(self):
		...
		for i in range(self.Npart):
			...
			if outOfBounds[i]:
				...
				if lenCrossed == 0:
					...
					if len(r1)>0:
						#consume by random selection among the sourc and drain
						...
						s2 = self.edgeStyle[self.edgeStyle[lineCrossed]<1]
						r2 = np.where(self.edgeStyle[lineCrossed]<1)[0]
						idxCross = r2[np.random.randint(len(r2))]
		...
		self.overlaps[squareform(np.invert(_overlaps))] = False
